      ...
      <div class="row align-items-center py-4">
        ...
        @foreach($buttons as $button)
            @if(isset($button['is_button']) && $button['is_button'] == true)
                <button type="button" {!! $button['components'] ?? '' !!} class="btn btn-sm btn-neutral {!! $button['class'] ?? '' !!}">{!! $button['name'] ?? '' !!}</button>
            @else
                <a href="{{ $button['url'] ?? '' }}" {!! $button['components'] ?? '' !!} class="btn btn-sm btn-neutral {!! $button['class'] ?? '' !!}">{!! $button['name'] ?? '' !!}</a>
            @endif
        @endforeach
    @endisset
        </div>
