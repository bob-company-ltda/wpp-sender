...
'title'   => __('Meta Campaign'),
'buttons' =>[
	[
	'name'=>'<i class="ni ni-calendar-grid-58"></i>&nbspCreate Campaign',
			...
			<div class="col">
				<div class="card card-stats">
					<div class="card-body">
						...
						</p><h5 class="card-title  text-muted mb-0">{{ __('Total Campaign') }}</h5>
						<p></p>
					</div>
				</div>
			</div>
			<div class="col">
				<div class="card card-stats">
					<div class="card-body">
						...
						<p class="mt-3 mb-0 text-sm">
						</p><h5 class="card-title  text-muted mb-0">{{ __('Total Pending Campaign') }}</h5>
						<p></p>
					</div>
				</div>
			</div>
			<div class="col">
				<div class="card card-stats">
					<div class="card-body">
						...
						<p class="mt-3 mb-0 text-sm">
						</p><h5 class="card-title  text-muted mb-0">{{ __('Campaign Executed') }}</h5>
						<p></p>
					</div>
				</div>
			</div>
			<div class="col">
				<div class="card card-stats">
					<div class="card-body">
						...
						<p class="mt-3 mb-0 text-sm">
						</p><h5 class="card-title  text-muted mb-0">{{ __('Failed Campaign') }}</h5>
						<p></p>
					</div>
				</div>
			</div>
			...
			<div class="col-sm-12">
				<div class="alert bg-gradient-primary text-white alert-dismissible fade show" role="alert">
					...
					<span class="alert-text">
						...
						{{ __('Run Campaign features is not available in your subscription plan') }}
										...
										<td>
											<div class="btn-group mb-2">
												...
												<div class="dropdown-menu">
													...
													<a class="dropdown-item has-icon" href="{{ route('user.schedule-message.show',$post->id) }}"><i class="ni ni-align-left-2"></i>{{ __('View Report') }}</a>

													@if($post->status == 'pending')
													<a class="dropdown-item has-icon delete-confirm" href="javascript:void(0)" data-action="{{ route('user.schedule-message.destroy',$post->id) }}"><i class="ni ni-basket"></i>{{ __('Remove Campaign') }}</a>
													
													@endif
													
													<a class="dropdown-item has-icon" href="{{ route('user.download.report',$post->id) }}"><i class="ni ni-shop"></i>{{ __('Download Report') }}</a>
												</div>
											</div>
										</td>
									</tr>
