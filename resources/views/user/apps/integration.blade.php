...
<div class="language-markup">
    <pre class="language-markup" tabindex="2">
<code class="language-markup">
<h3>{{ __('Meta Approved templates with Variables') }}</h3>    
curl --location --request POST '{{ $url }}' \
--form 'appkey="{{ $key }}"' \
--form 'authkey="{{ Auth::user()->authkey }}"' \
--form 'to="RECEIVER_NUMBER"' \
--form 'template_id="TEMPLATE_ID"' \
--from 'header="PARAMETER VALUE"'\  ({1})
--from 'body="PARAMETER VALUES WITH COMMA"'\ ({1},{2},{3})
...
curl_setopt_array($curl, array(
  ...
  'body' => 'PARAMETER VALUE WITH COMMA',
  ),
));
