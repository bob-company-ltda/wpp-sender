        ...
        <form class="ajaxform" action="{{ route('admin.developer-settings.update',$id) }}">
            ...
            <div class="card-body">
                ...
                <div class="from-group row mt-2">
                    <label class="col-lg-12">{{ __('Email Verification System') }}</label>
                    <div class="col-lg-12">
                        <select class="form-control" name="mail_verification">
                            <option value="true" {{ env('MAIL_VERIFICATION') == true ? 'selected' : '' }}>{{ __('Enable') }}</option>
                            <option value="false" {{ env('MAIL_VERIFICATION') == false ? 'selected' : '' }}>{{ __('Disable') }}</option>
                        </select>
                    </div>
                </div>
                <div class="from-group row mt-2">
