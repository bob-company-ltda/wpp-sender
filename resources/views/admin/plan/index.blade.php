...
<div class="col-md-6 col-lg-3 text-center">
    <div class="card">
        <div class="card-body">
            ...
            {{ $plan->days == 30 ? 'Per month' : ($plan->days == 90 ? 'Per quarter' : ($plan->days == 180 ? 'Per half year' : 'Per year')) }}
            <br>
            <span href="#!" class="text-muted">{{ __('Active Users') }} ({{ $plan->activeuser_count }})</span>
            <hr>
            @foreach($plan->data ?? [] as $key => $data)
                @if($key !== 'cloudapi_limit' && $key !== 'access_group_list' && $key !== 'mechanism') <!-- Added mechanism -->
