$(document).ready(function() {
    ...
    $(".delete-note").off('click').on('click', function (event) {
        ...
        var noteId = deleteButton.parents('.note-item').data('note-id');
        $.ajax({
            url: base_url + '/user/notes/' + noteId,
            type: 'DELETE',
            success: function (response) {
                console.log(response.message);
                deleteButton.parents('.note-item').remove();
            },
            error: function (xhr) {
    ...
    function updateFavoriteStatus(noteId) {
    $.ajax({
        url: base_url + '/user/notes/' + noteId + '/update-favorite',
        type: 'PUT',
        success: function (response) {
            console.log(response.message);
        },
    ...
    $(".fav-note").off('click').on('click', function(event) {
        event.stopPropagation();
        var noteItem = $(this).parents('.note-item');
        
        // Toggle the 'note-fav' class immediately
        noteItem.toggleClass('note-fav');

        // Get the note ID from the note item's data attribute or another way
        var noteId = $(this).parents('.note-item').data('note-id'); // Adjust based on your data attribute

        // Call the function to update favorite status
        updateFavoriteStatus(noteId);
    })
}

function updateTag(noteId, tag) {
    $.ajax({
        url: base_url + '/user/notes/' + noteId + '/update-tag',
        type: 'PUT',
        data: { tag: tag },
        success: function (response) {
            console.log(response.message);
    ...
    $('.tags-selector .label-group-item').off('click').on('click', function(event) {
        ...
        var tag = $(this).data('tag'); // Assuming the data attribute stores the numeric tag value

        var noteItem = $(this).parents('.note-item');
        noteItem.data('tag', tag); // Store the tag value in a data attribute
        ...
        // Get the note ID from the note item's data attribute or another way
        var noteId = noteItem.data('note-id'); // Adjust based on your data attribute

        // Call the function to update the tag
        updateTag(noteId, tag);
    });
}
    ...
    $("#btn-n-add").on('click', function(event) {
        ...
        error: function (xhr) {
            // Handle errors, e.g., show validation errors, etc.
